...
unordered_map<string, int> _PT = {
    {"+", 37},    //把两个操作数相加
    {"-", 38},    //从第一个操作数中减去第二个操作数
    {"*", 39},    //把两个操作数相乘 或者 指向一个变量
    {"/", 40},    //分子除以分母
    {"%", 41},    //取模运算符，整除后的余数
    {"++", 42},   //自增运算符，整数值增加1
    {"--", 43},   //自减运算符，整数值减少1
    {"==", 44},   //检查两个操作数的值是否相等，如果相等则条件为真
    {"!=", 45},   //检查两个操作数的值是否相等，如果不相等则条件为真
    {">", 46},    //检查左操作数的值是否大于右操作数的值，如果是则条件为真
    {"<", 47},    //检查左操作数的值是否小于右操作数的值，如果是则条件为真
    {">=", 48},   //检查左操作数的值是否大于或等于右操作数的值，如果是则条件为真
    {"<=", 49},   //检查左操作数的值是否小于或等于右操作数的值，如果是则条件为真
    {"&&", 50},   //称为逻辑与运算符。如果两个操作数都非零，则条件为真
    {"||", 51},   //称为逻辑或运算符。如果两个操作数中有任意一个非零，则条件为真
    {"!", 52},    //称为逻辑非运算符。用来逆转操作数的逻辑状态。如果条件为真则逻辑非运算符将使其为假
    {"&", 53},    //按位与操作，按二进制位进行"与"运算 或者 返回变量的地址
    {"|", 54},    //按位或运算符，按二进制位进行"或"运算
    {"^", 55},    //异或运算符，按二进制位进行"异或"运算
    {"~", 56},    //取反运算符，按二进制位进行"取反"运算
    {"<<", 57},   //二进制左移
    {">>", 58},   //二进制右移
    {"=", 59},    //简单的赋值运算符，把右边操作数的值赋给左边操作数
    {"+=", 60},   //加且赋值运算符，把右边操作数加上左边操作数的结果赋值给左边操作数
    {"-=", 61},   //减且赋值运算符，把左边操作数减去右边操作数的结果赋值给左边操作数
    {"*=", 62},   //乘且赋值运算符，把右边操作数乘以左边操作数的结果赋值给左边操作数
    {"/=", 63},   //除且赋值运算符，把左边操作数除以右边操作数的结果赋值给左边操作数
    {"%=", 64},   //求模且赋值运算符，求两个操作数的模赋值给左边操作数
    {"<<=", 65},  //左移且赋值运算符
    {">>=", 66},  //右移且赋值运算符
    {"&=", 67},   //按位与且赋值运算符
    {"^=", 68},   //按位异或且赋值运算符
